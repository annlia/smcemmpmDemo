...
datafile <- "simPbit8Data1k.RData" ### file with data to model
tosave <- TRUE
savefile <- "smcemPbitDemo11062013.RData" ### file to save R workspace after processing
...
### The case when constrained&&(!useinvariance) == TRUE 
### corresponds to imposing the covariance matrix to be in correlation form
...
### condition to check when no correlation form is imposed
...
for(j in 1:nobs){
  ...
  obsInfo <- fisherDiag(updbe, updsig, cityX[[j]], samp$samp, samp$W)
