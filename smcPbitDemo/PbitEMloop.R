...
### One step of the SMCEM procedure for a Probit Model 
...
PbitEMloop <- function(obs, X, p, ncov, estbe = rep(0.5,ncov+1), prev.be, sigem = diag(p), prev.sigem, smcM = 1000){
	### evaluate number of observations
	nobs <- length(obs[1,])
	### define constraints
	best <- matrix(0, nrow = p, ncol = nobs)
	for(i in 1:nobs) best[,i] <- X[[i]]%*%estbe
### define previous constraints
	prev.best <- matrix(0, nrow = p, ncol = nobs)
	for(i in 1:nobs) prev.best[,i] <- X[[i]]%*%prev.be
...
calctime<-0
	...
	for(l in 1:nobs){
########################################		
### Modification for sequential mode ###
### estobs <- obseval(obs[,l], best[,l], sigem, smcM)
### Load previous sample
loadingtimetemp<-proc.time()

		nameSample <- paste("samples/sample",l,".RData", sep="")
		load(nameSample) ### contains variable estobs, with names: prob, W, samp
        
### Implement "one step" SMC ###########
		estobs <- smcEM(obs[,l], best[,l], prev.best[,l], sigem, prev.sigem, estobs, estobs$sf)
		save(estobs, file=nameSample)
